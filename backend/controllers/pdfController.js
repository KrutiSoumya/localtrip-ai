...
const generateItineraryPDF = async (req, res) => {
  try {
    ...
    const data = {
      ...
      total_budget:
        latestQuotation?.total_price ||
        itinerary.lead.budget ||
        0,

      currency: latestQuotation?.currency || "INR",

      ai_confidence_label:
        itinerary.lead.score_label || "N/A",
    };
    ...
    res.set({
      ...
      "Content-Disposition":
        `attachment; filename=itinerary-${itinerary.id}.pdf`,
    });

    res.send(pdfBuffer);

  } catch (error) {
    console.error("Itinerary PDF error:", error);
...
const generateQuotationPDF = async (req, res) => {
  try {
    ...
    const data = {
      agency_name:
        agentConfig?.name || "LocalTrip Agency",
      ...
      customer_name:
        quotation.itinerary.lead.customer_name,

      destination:
        quotation.itinerary.lead.destination || "N/A",

      group_size:
        quotation.itinerary.lead.group_size || "N/A",

      accommodation_cost:
        breakdown.accommodation || "N/A",

      transport_cost:
        breakdown.transport || "N/A",

      activities:
        breakdown.activities || [],

      profit_margin:
        breakdown.profit_margin || "N/A",

      grand_total:
        `${quotation.currency} ${quotation.total_price}`,
      ...
      agent_name:
        agentConfig?.name || "LocalTrip Agent",
    };

    const pdfBuffer =
      await generatePDF("quotation", data);
...
const generateBookingConfirmationPDF = async (
  req,
  res
) => {
  try {
    ...
    const data = {
      ...
      customer_name:
        booking.lead.customer_name,

      destination:
        booking.lead.destination || "N/A",

      itinerary_title:
        booking.itinerary.title,

      total_price:
        `${booking.quotation.currency} ${booking.quotation.total_price}`,
    ...
    res.set({
      "Content-Type": "application/pdf",
      "Content-Disposition":
        `attachment; filename=booking-confirmation-${booking.id}.pdf`,
    });
    ...
    console.error(
      "Booking confirmation PDF error:",
      error
    );

    res.status(500).json({
      message:
        "Error generating booking confirmation PDF",
      error: error.message,
    });
  }
};
